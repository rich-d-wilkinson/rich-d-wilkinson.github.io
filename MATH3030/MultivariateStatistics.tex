...
\(\{\boldsymbol x_i,\boldsymbol y_i\}_{i=1}^n.\)
...
Canonical correlation analysis (CCA) gives an answer to this question in terms of the best low-dimensional linear projections of the \(\boldsymbol x\) and \(\boldsymbol y\) random variables. In a comparable way to PCA, `best' in CCA is defined in terms of maximizing correlations.
...
\begin{itemize}
\item
  Let \(\bar{\boldsymbol x}\), \(\bar{\boldsymbol y}\), and \(\bar{\boldsymbol z}\) denote the sample mean vectors of the \(\boldsymbol x_i\), \(\boldsymbol y_i\) and \(\boldsymbol z_i\) respectively.
\item
...
In other words:
\begin{align}
&\mbox{Maximise} \qquad\qquad \quad  r(\boldsymbol a^\top \boldsymbol x,\boldsymbol b^\top \boldsymbol y),
\label{eq:opt26}\\
...
Intuitively, this objective makes sense, because we want to find the linear combination of the \(\boldsymbol x\)-variables and the linear combination of the \(\boldsymbol y\)-variables which are most highly correlated.
...
\begin{align}
  ...
  \label{eq:invar}
  \end{align}
\(r(\boldsymbol a^\top \boldsymbol x,\boldsymbol b^\top \boldsymbol y)\) is invariant to (i.e.~unchanged by) multiplication of \(\boldsymbol a\) and \(\boldsymbol b\) by positive scalars. Consequently there will be an infinite number of solutions to this optimisation problem, because if \(\boldsymbol a\) and \(\boldsymbol b\) are solutions, then so are \(\gamma \boldsymbol a\) and \(\delta \boldsymbol b\), for any \(\gamma>0\) and \(\delta>0\).

A more useful way to formulate this optimisation problem is

\begin{align}
&\mbox{Maximize }\qquad \boldsymbol a^\top \boldsymbol S_{\boldsymbol x\boldsymbol y}\boldsymbol b
\label{eq:opt27a}\\
...
Thankfully, there is a link between the solutions of the two optimization problems \eqref{eq:opt26} and \eqref{eq:opt27a}. Firstly, the invariance of \(r(\boldsymbol a^\top \boldsymbol x, \boldsymbol b^\top \boldsymbol y)\) means that if
\(\check{\boldsymbol a}\) and \(\check{\boldsymbol b}\) are a solution to problem \eqref{eq:opt27a}, then for any \(\gamma,\;\delta>0\), we have that \(\boldsymbol a=\gamma \check{\boldsymbol a}\) and \(\boldsymbol b=\delta \check{\boldsymbol b}\) are a solution to \eqref{eq:opt26}.
...
Substituting \(\boldsymbol a=\boldsymbol Q\boldsymbol b/\lambda\) into \eqref{eq:ccaevp2} gives
...
to stress that \(\boldsymbol a_1\) and \(\boldsymbol b_1\) are the \textbf{first} pair of \textbf{canonical correlation (CC) vectors}. The variables \(\eta_1=\boldsymbol a_1^\top (\boldsymbol x-\bar{\boldsymbol x})\) and \(\psi_1=\boldsymbol b_1^\top (\boldsymbol y-\bar{\boldsymbol y})\) are called the first pair of \textbf{canonical correlation variables}, and \(\sigma_1={\mathbb{C}\operatorname{ov}}(\eta_1, \psi_1)\) is the \textbf{first canonical correlation}.
...
\boldsymbol b_1 &=\boldsymbol S_{\boldsymbol y\boldsymbol y}^{-1/2}\boldsymbol v_1\\
...
We can see that \(\psi_1\) is measuring something similar to goal difference \(G-GA\), as usually defined, but it gives higher weight to goals conceded than goals conceded (\(0.397\) versus \(-0.397\)).
...
Let us first recap what we did in the previous section: we found a linear combination of the \(\boldsymbol x\)-variables and a linear combination of the \(\boldsymbol y\)-variables which
...
\mbox{ and }\;\;\;&\boldsymbol a_i^\top \boldsymbol S_{xx}\boldsymbol a=\boldsymbol b_i^\top \boldsymbol S_{yy}\boldsymbol b= 0 \mbox{ for }i=1, \ldots, k-1
\end{align}
...
```\{proof\}\textbf{Proof of Proposition \ref{prp:ccafull}}. We note as before that if we write \(\tilde{\boldsymbol a}_j =\boldsymbol S_{\boldsymbol x\boldsymbol x}^{1/2} \boldsymbol a_j\) and \(\tilde{\boldsymbol b}_j=\boldsymbol S_{\boldsymbol y\boldsymbol y}^{1/2} \boldsymbol b_j\), then the constraints become
...
\[
\boldsymbol a_2=\boldsymbol S_{\boldsymbol x\boldsymbol x}^{-1/2} \boldsymbol q_2 = \begin{pmatrix}0.0313 \\0.329 \\\end{pmatrix}
\qquad \text{and} \qquad
\boldsymbol b_2=\boldsymbol S_{\boldsymbol y\boldsymbol y}^{-1/2}\boldsymbol r_2=\begin{pmatrix}-0.07 \\-0.104 \\\end{pmatrix},
\]
with standardised versions (without the sign changes this time)
\[
\boldsymbol a_2=\begin{pmatrix}0.0947 \\0.996 \\\end{pmatrix}
\qquad \text{and} \qquad
\boldsymbol b_2=\begin{pmatrix}-0.56 \\-0.829 \\\end{pmatrix},
\]
and new variables
\[
\eta_2 =0.0947 (W-\bar{W}) +0.996(D -\bar{D})
\]
and
\[
 \psi_2 = -0.56(G-\bar{G}) -0.829(GA-\bar{GA}).
\]
...
Although CCA is clearly a different technique to linear regression, it turns out that when either \(\dim \boldsymbol x=p=1\) or \(\dim \boldsymbol y=q=1\), there is a close connection between the two approaches.
...
\[\boldsymbol S_{xx}=\frac{1}{n}\boldsymbol X^\top \boldsymbol X\quad \mbox{and}\quad \boldsymbol S_{xy}=\frac{1}{n}\boldsymbol X^\top \boldsymbol y,\] and the second means that \((\boldsymbol X^\top \boldsymbol X)^{-1}\) exists.
...
Firstly, since CCA depends only on sample covariance matrices, it follows that the translation vectors \(\pmb \mu\) and \(\pmb \eta\) have no effect on the analysis.

Secondly, let's consider the effect of the rotations by an orthogonal matrix. We've seen that CCA in the original \(\boldsymbol x\) and \(\boldsymbol y\) coordinates depends on
\begin{equation}
...
But can we do the reverse and reconstruct the map from the distance matrix? This is the aim of multidimensional scaling.
...
represent the points in a low-dimensional space will make visualization and statistical analysis easier. We shall also see, perhaps unsurprisingly, that there is a close connection between MDS and PCA.

\hypertarget{notation-2}{%
\paragraph{Notation}\label{notation-2}}
\addcontentsline{toc}{paragraph}{Notation}

\begin{definition}
\protect\hypertarget{def:distanceD}{}{\label{def:distanceD} }The \(n \times n\) matrix \(\boldsymbol D=(d_{ij})_{i,j=1}^n\) is a \textbf{distance matrix} (sometimes called a \textbf{dissimilarity matrix}) if
...
\item
  \(\boldsymbol D=\boldsymbol D^\top\), i.e., \(\boldsymbol D\) is symmetric (\(d_{ij}=d_{ji}\)).
\item
  \(d_{ij}=0\) implies \(\boldsymbol x_i=\boldsymbol x_j\) DO I NEED THIS??????????? MDS STARTS WITH D- there is no x.
\end{enumerate}
\end{definition}

We do not require distances to necessarily satisfy the triangle inequality
\begin{equation}
...
\[d(\boldsymbol x, \boldsymbol x') = ||\boldsymbol x-\boldsymbol x'||_2 = \sqrt{(\boldsymbol x-\boldsymbol x')^\top (\boldsymbol x-\boldsymbol x')}.\]
...
\begin{definition}
\protect\hypertarget{def:euclideanD}{}{\label{def:euclideanD} }Suppose \(\boldsymbol D\) is an \(n\times n\) distance matrix. We say \(\boldsymbol D\) is a \textbf{Euclidean distance matrix}
if there is a set of points \(\boldsymbol x_1,\ldots , \boldsymbol x_n \in \mathbb{R}^p\) for some \(p\), such that
\[
d_{ij}=\vert \vert \boldsymbol x_i -\boldsymbol x_j \vert \vert_2 =\sqrt{(\boldsymbol x_i-\boldsymbol x_j)^\top (\boldsymbol x_i-\boldsymbol x_j)}.
\]
\end{definition}
...
\begin{definition}
\protect\hypertarget{def:unnamed-chunk-3}{}{\label{def:unnamed-chunk-3} }Given a distance matrix \({\mathbf D}=\{d_{ij}\}_{i,j=1}^n\), define \(\boldsymbol A\) to be the matrix of negative square distances divided by two:
\begin{equation}
\boldsymbol A=\{a_{ij}\}_{i,j=1}^n,  \quad \text{where} \qquad a_{ij}=-\frac{1}{2}d_{ij}^2.
\label{eq:defA}
\end{equation}

The \textbf{centred inner-product matrix} (also called the centred-Gram matrix) is
\begin{equation}
...
\end{equation*}
is the \(n \times n\) \textbf{centering matrix} (see Section \ref{centering-matrix}).
\end{definition}

The reason we call \(\boldsymbol B\) the centred inner product matrix will become clear later.

\hypertarget{main-result}{%
\subsubsection*{Main result}\label{main-result}}
\addcontentsline{toc}{subsubsection}{Main result}

We now present the key result for classical MDS. It says that a distance matrix \(\boldsymbol D\) is a \emph{Euclidean} distance matrix if and only if the corresponding centred inner product matrix \(\boldsymbol B\) is positive semi-definite. It also tells us how given data \(\boldsymbol X\) we can compute \(\boldsymbol B\) directly from \(\boldsymbol X\). Conversely, and more importantly, it tells us how given \(\boldsymbol B\) we can compute some data points \(\boldsymbol X\) that have corresponding Euclidean distance matrix \(\boldsymbol D\).

\begin{theorem}
\protect\hypertarget{thm:five1}{}{\label{thm:five1} }Let \(\boldsymbol D\) denote an \(n \times n\) distance matrix with corresponding centered inner-product matrix \(\boldsymbol B=\boldsymbol H\boldsymbol A\boldsymbol H.\)

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  If \(\boldsymbol D\) is a \textbf{Euclidean} distance matrix for the sample of \(n\) vectors \(\boldsymbol x_1,\ldots , \boldsymbol x_n\), then
  \begin{equation}
  \boldsymbol B= ({\mathbf H} {\mathbf X})({\mathbf H} {\mathbf X})^\top.
  \label{eq:bijB}
  \end{equation}
  Thus \(\boldsymbol B\) is positive semi-definite.
\item
  Suppose \(\boldsymbol B\) is positive semi-definite with eigenvalues \(\lambda_1 \geq \lambda_2 \cdots \geq \lambda_k \geq 0\) and that it has spectral decomposition \(\boldsymbol B={\mathbf U} {\pmb \Lambda}{\mathbf U}^\top\), where \({\pmb \Lambda}=\text{diag}\{\lambda_1 \ldots \lambda_k\}\) and \(\mathbf U\) is \(n \times k\) and satisfies \({\mathbf U}^\top {\mathbf U}={\mathbf I}_k\). Then
  \[{\mathbf X}=[\boldsymbol x_1, \ldots , \boldsymbol x_n]^\top={\mathbf U}{\pmb \Lambda}^{1/2}\]
  is an \(n \times k\) data matrix which has \textbf{Euclidean} distance marrix \(\boldsymbol D\).

  Moreover, for this data matrix \(\bar{\boldsymbol x}={\mathbf 0}_k\) and \(\boldsymbol B\) represents the inner product matrix with elements given by \eqref{eq:bijB}.
\end{enumerate}
\end{theorem}
...
\end{equation}
where \(\bar{\mathbf x}=n^{-1}\sum_{i=1}^n \boldsymbol x_i\) is the sample mean vector.
This illustrates why we call \(\boldsymbol B\) the \emph{centred inner-product} matrix: \(\boldsymbol H\boldsymbol X\) is the centred data matrix, and \((\boldsymbol H\boldsymbol X)(\boldsymbol H\boldsymbol X)^\top\) thus contains the inner product of each pair of centred vectors.
...
\begin{corollary}
\protect\hypertarget{cor:unnamed-chunk-5}{}{\label{cor:unnamed-chunk-5} }The distance matrix \(\boldsymbol D\) is a Euclidean distance matrix if and only if the corresponding centred inner-product matrix \(\boldsymbol B\) is a positive semi-definite matrix.
\end{corollary}

\hypertarget{important-point}{%
\paragraph{Important Point}\label{important-point}}
\addcontentsline{toc}{paragraph}{Important Point}

Proposition \ref{thm:five1} may be useful even if \({\mathbf D}\) is not a \emph{Euclidean} distance matrix (in which case \(\boldsymbol B\) will have some negative eigenvalues).

Instead of using \(\boldsymbol B\) in Proposition \ref{thm:five1} we can use its positive part. If \(\boldsymbol B\) has spectral decomposition \(\sum_{j=1}^p \lambda_j \boldsymbol u_j \boldsymbol u_j^\top\), then its positive definite part is defined by
\[
\boldsymbol B_{\text{pos}}=\sum_{j: \, \lambda_j>0} \lambda_j \boldsymbol u_j \boldsymbol u_j^\top.
\]
In other words, we sum over those \(j\) such that \(\lambda_j\) is positive.

Then \(\boldsymbol B_{\text{pos}}\) is positive semi-definite and so we can use part 2. of Theorem \ref{thm:five1} to determine a Euclidean configuration which has centred inner-product matrix \(\boldsymbol B_{\text{pos}}\). Then, provided the negative eigenvalues are small in absolute value relative to the positive eigenvalues, the inter-point distances of the new points in Euclidean space should provide a good approximation to the original inter-point distances \((d_{ij})\).
...
You should check that \(\boldsymbol H\boldsymbol A\boldsymbol H\) is the same as \((\boldsymbol H\boldsymbol X)(\boldsymbol H\boldsymbol X)^\top\), thus verifying part 1. of Theorem \ref{thm:five1}. Note that there will be very small differences (\(\sim 10^{-16}\)) due to numerical errors.

Can we now find a set of \(5\) points in \(\mathbb{R}^2\) that have a Euclidean distance matrix \(\boldsymbol D\) if we are only given the matrix \(\boldsymbol B\)?
...
Firstly, we need to compute the spectral decomposition of \(\boldsymbol B\). The eigenvalues of \(\boldsymbol B\) are
\[
\lambda_1=\lambda_2=2 \qquad \text{and} \qquad \lambda_3=\lambda_4=\lambda_5=0.
\]
Note that, as expected from Proposition \ref{thm:five1}, \(\boldsymbol B\) is positive semi-definite because it is a Euclidean
distance matrix.

Because we have a repeated eigenvalue (\(\lambda=2\) has \emph{multiplicity} 2), the eigenspace associated with \(\lambda=2\) is a two dimensional space, and so there are many pairs of mutually orthogonal unit eigenvectors spanning this eigenspace (i.e., there is not a unique pair of eigenvectors). One choice (the sparsest) is
\[
\boldsymbol u_1= \begin{pmatrix}0 \\ 0 \\ -\frac{1}{\sqrt{2}} \\ 0 \\ -\frac{1}{\sqrt{2}} \end{pmatrix} \qquad
\text{and} \qquad \boldsymbol q_2 =\begin{pmatrix}0 \\ -\frac{1}{\sqrt{2}} \\ 0 \\ \frac{1}{\sqrt{2}}\\ 0 \end{pmatrix}.
\]
...
We can compute the coordinates of five points in \(\mathbb{R}^2\) which have Euclidean distance matrix, \(\boldsymbol D,\) by
\[
\boldsymbol Q\boldsymbol \Lambda^{1/2}=\sqrt{2}[\boldsymbol q_1 , \boldsymbol q_2].\]

Note again that these coordinates are not unique as any rotation or translation of them will have the same distance matrix. In particular, when doing computations in R we may find different answers depending upon how we do the computation. So for example, if we use the eigendecompostion of \(\boldsymbol B= \boldsymbol H\boldsymbol A\boldsymbol H\)
...
Where as if we use the eigen decomposition of \((\boldsymbol H\boldsymbol H)(\boldsymbol H\boldsymbol X)^\top\) we find a different set of points:
...
The distance matrices for both sets of points are equal to \(\boldsymbol D\)
...
If we plot the original data points \(\boldsymbol X\), along with the three sets of reconstructed data points \texttt{Y}, \texttt{Y2}, and \texttt{Y.mds} we can see that they are essentially the same, apart from being rotatated or reordered.
...
Let's now look at a case for which \(\boldsymbol B\) isn't positive dinite. We'll create this by modifying the distance matrix we had before:

\[\boldsymbol D_2 = \begin{pmatrix}0&0.5&1&1&1 \\0.5&0&1.41&2&1.41 \\1&1.41&0&1.41&2 \\1&2&1.41&0&1.41 \\1&1.41&2&1.41&0 \\\end{pmatrix}.\]
This is a distance matrix, but is it a \emph{Euclidean} distance matrix? I.e., is there a set of vectors \(\boldsymbol x_1, \ldots, \boldsymbol x_5\) which have Euclidean distances between them given by \(\boldsymbol D_2\)?

If we look at the eigenvalues of the centred inner-product matrix \(\boldsymbol B\) associated with \(\boldsymbol D_2\), then we find that it has negative eigenvalues, and is thus not positive semi-definite. Thus by Theorem \ref{thm:five1} \(\boldsymbol D_2\) is not a \emph{Euclidean} distance matrix.
...
\CommentTok{# note D^2 does element wise operations, different to D%*%D}
\NormalTok{B2 <-}\StringTok{ }\NormalTok{H}\OperatorTok{%*%}\StringTok{ }\NormalTok{A2}\OperatorTok{%*%}\NormalTok{H }\CommentTok{#check this matches  (H %*% X) %*% t(H %*% X)}
\KeywordTok{eigen}\NormalTok{(B2)}\OperatorTok{$}\NormalTok{values}
\end{Highlighting}
\end{Shaded}
...
We can still use classical multidimensional scaling to find a set of points \(\boldsymbol x_1, \ldots, \boldsymbol x_5\) that have distances approximately given by \(\boldsymbol D_2\).
\texttt{cmdscale} allows us to specify the dimension of the points \(\boldsymbol x_i\). If we pick \(\dim(\boldsymbol x)=2\) then itt gives us the set of points
...
which have a distance matrix that approximates \(\boldsymbol D_2\), but does not equal it. By using a different number of dimensions, we may be able to get a set of points have a distance matrix closer to \(\boldsymbol D\), but we will not be able to find a set of \(5\) points that have a distance matrix exactly equal to \(\boldsymbol D_2\).
...
\NormalTok{Lambda_sqrt <-}\StringTok{ }\KeywordTok{diag}\NormalTok{(}\KeywordTok{eigen}\NormalTok{(B2)}\OperatorTok{$}\NormalTok{values[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{])}
\NormalTok{U }\OperatorTok{%*%}\StringTok{ }\NormalTok{Lambda_sqrt}
\end{Highlighting}
\end{Shaded}
...
Finally, if we apply the \texttt{cmdscale} to the distances between UK cities from the introduction, we get the `coordinates' shown below.
...
Starting with a distance matrix \(\boldsymbol D\), and using the matrix \(\boldsymbol B\), we now show how to calculate exact or approximate Euclidean coordinates for the \(n\) objects under study. We already know from Proposition \ref{thm:five1} how to do this when the distance matrix \(\boldsymbol D\) is Euclidean, but we will see now that this construction works more generally. Moreover, there is a very close connection with principal components analysis.
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-21}{}{\label{exm:unnamed-chunk-21} }Suppose there are 4 attributes we wish to consider.
...
\begin{proposition}
\protect\hypertarget{prp:mds2}{}{\label{prp:mds2} }Suppose that \(\boldsymbol F\) is a similarity matrix. If, in addition, \(\boldsymbol F\) is positive semi-definite, then \(\boldsymbol D\) defined in \eqref{eq:defD} is Euclidean with centred inner product matrix
\begin{equation}
...
\begin{proof}
\iffalse{} {Proof. } \fi{}Since \(\boldsymbol F\) is positive semi-definite by assumption, and \(\boldsymbol H^\top =\boldsymbol H\) by definition of \(\boldsymbol H\), it follows that \(\boldsymbol H\boldsymbol F\boldsymbol H\) must also be positive semi-definite. So by Result 5.1, we just need to show that \eqref{eq:BHFH} holds, where \(\boldsymbol B\) is given by \(\boldsymbol B=\boldsymbol H\boldsymbol A\boldsymbol H\) and \(\boldsymbol A\) is defined as in \eqref{eq:defA}, and the \(d_{ij}\) are defined by \eqref{eq:defD}. Then
\[
...
\item
  ...
  \item
    Show that \(\boldsymbol B\) is positive semi-definite (in which case part (ii) of Result 5.1 holds). \textbf{Hint}: explain why, for
...
\def\labelenumi{\arabic{enumi}.}
\item
  If \(M \sim W_p(\boldsymbol \Sigma,n)\), prove that \({\mathbb{E}}\boldsymbol M= n \boldsymbol \Sigma\).
\item
...
\item
  A survey of \(n=25\) families records the head length of the first (\(x_1\)) and second (\(x_2\)) sons. The sample mean is \(\bar{\boldsymbol x} = (185.72, 183.84)^\top\). Assume that the observations are sampled from \(N_2(\boldsymbol \mu,\boldsymbol \Sigma)\) where \(\boldsymbol \Sigma= \text{diag}(100,100)\).
  ...
  \end{itemize}
\item
  Let \(\boldsymbol x_1, \ldots, \boldsymbol x_{20}\) be a random sample of vectors from a \(N_3(\boldsymbol \mu,\boldsymbol \Sigma)\) population where \(\boldsymbol \mu\) and \(\boldsymbol \Sigma\) are unknown. The sample mean and sample covariance matrix are given by
  \[\bar{\boldsymbol x} = \begin{pmatrix} 0.358 \\ -1.056 \\ -1.795 \end{pmatrix} \qquad
  \boldsymbol S= \begin{pmatrix} 0.522 & 0.556 & -2.285 \\ 0.556 & 3.258 & -0.765 \\ -2.285 & -0.765 & 14.093 \end{pmatrix}.\]

  \begin{itemize}
  \tightlist
  \item
    Use Hotelling's \(T^2\) distribution to perform a significance test of the hypothesis \(H_0: \boldsymbol \mu= (0,-1,-1)^T\). Note that \(\boldsymbol S= \boldsymbol Q\boldsymbol \Lambda\boldsymbol Q^T\) where \(\boldsymbol \Lambda= \text{diag}(14.531, 3.253,0.090)\) and
    \[\boldsymbol Q= \begin{pmatrix} -0.163 & -0.121 & -0.979 \\ -0.075 & -0.988 & 0.135 \\ 0.984 & -0.095 & -0.152 \end{pmatrix}.\]
  \item
...
\item
  ...
  The sample means are \(\bar{\boldsymbol x}_1 = (181.50,129.17)^\top\) and \(\bar{\boldsymbol x}_2 = (205.06,120.44)^\top\); and the sample covariance matrices are
  \[\boldsymbol S_1 = \begin{pmatrix} 120.58 & 56.25 \\ 56.25 & 44.63 \end{pmatrix} \qquad
  \boldsymbol S_2 = \begin{pmatrix} 203.94 & 73.42 \\ 73.42 & 47.14 \end{pmatrix}.\]
...
\setcounter{enumi}{1}
\item
  The command \texttt{rWishart} can be used to simulate from a Wishart distribution. Alternatively, to sample from \(W_p(\boldsymbol \Sigma,n)\), you can sample \(\boldsymbol x_1, \ldots, \boldsymbol x_n \sim N_p(\boldsymbol 0, \boldsymbol \Sigma)\) and set \(\boldsymbol M= \sum \boldsymbol x_i \boldsymbol x_i^\top\).
  ...
  \item
    Set \(\boldsymbol a= (1\; 1)^\top\). Check empirically that \(\boldsymbol a^\top \boldsymbol M\boldsymbol a\sim 3 \chi^2_10\). \textbf{Hint} plot the theoretical densities on top of a histogram of the sampled quantities.
  \item
    Suppose \(\boldsymbol x_1, \ldots, \boldsymbol x_{10} \sim N_2(\boldsymbol 0, \operatorname{diag}(2,1))\). Empirically check that Proposition \ref{prp:six12} is true by computing the covariance matrix of a large number of such samples, and comparing this to the mean and variance of the Wishart distribution specified in the proposition.
  \item
...
\item
  ...
  \item
    ...
    \[x^2+y^2=c^2\]
    or in vector form:
    \[\boldsymbol x^\top\boldsymbol x=c^2.\]
