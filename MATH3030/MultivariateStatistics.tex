...
\textbf{Warning:} these lecture notes are still in preparation. Chapters 1-8 have been finished. Chapters 9-10 are still being worked on.
...
\hypertarget{lm}{%
\chapter{The Multivariate Linear Model}\label{lm}}

ADD TOTAL LEAST SQUARES (with SVD) + std least squares with SVD?

In the standard linear model, the response variable, \(y\), is univariate and the mean of \(y\), \(\mu=E[y]\), is modelled as a linear function of the elements of a covariate vector \(\mathbf x=(x_1, \ldots , x_q)^\top \in \mathbb{R}^q\), i.e.~it is assumed that
\[
\mu = \boldsymbol \beta^\top \mathbf x,
\]
where \(\boldsymbol \beta\in \mathbb{R}^q\) is an unknown parameter vector to be estimated. In this chapter we consider a generalisation of the standard linear model in which the response, \(\mathbf y\), is now a \(p \times 1\) vector. In this setting, the linear model takes the form
\[
{\pmb \mu}=\mathbf B^\top \mathbf x,
\]
where the mean vector \({\boldsymbol{\mu}}\) is \(p \times 1\), the covariate vector \(\mathbf x\) is \(q \times 1\) and the parameter vector \(\mathbf B\) is \(q \times p\). The reason for having \(\mathbf B^\top\) above rather than simply \(\mathbf B\) will become clear later in the chapter.
...
\label{eq:slm1}
\end{equation}
where \(\stackrel{q \times 1}{\boldsymbol \beta}\) is a parameter vector and \(\mathbf x_i\) is a \(q \times 1\) covariate vector for experimental unit \(i\). We can also write \eqref{eq:slm1} in equivalent vector-matrix form
\begin{equation}
\mathbf y=\mathbf X\boldsymbol \beta+{\pmb \epsilon},
\label{eq:slm2}
\end{equation}
where \(\stackrel{n \times q}{\mathbf X}=[\mathbf x_1 , \ldots , \mathbf x_n]^\top\) is the matrix of covariates,
...
It is assumed throughout this chapter that \(\mathbf X\) has full rank \(p\), so that \((\mathbf X^\top \mathbf X)^{-1}\) exists.
...
\]
Setting \(\partial \ell(\hat{\boldsymbol \beta}, \hat{\sigma}^2))/\partial \boldsymbol \beta={\mathbf 0}_q\), the zero vector, and assuming that \(\mathbf X^\top \mathbf X\) is invertible, implies that
\begin{equation}
...
In the standard linear model the responses \(y_i\) are univariate. In the multivariate linear model, the responses are \(p \times 1\) vectors \(\mathbf y_i\). Here, the linear model takes the form
\begin{equation}
\mathbf y_i=\mathbf B^\top \mathbf x_i +{\pmb \epsilon}_i, \qquad i=1, \ldots , n,
\label{eq:mlm1}
\end{equation}
where \(\stackrel{q \times p}{\mathbf B}\) is a parameter matrix, the \(\mathbf x_i\) are \(q \times 1\) covariate vectors as in \S 8.2, and the \({\pmb \epsilon}_i\) are \(p \times 1\) error vectors. The model \eqref{eq:mlm1} may be written in matrix form as
\begin{equation}
\mathbf Y= \mathbf X\mathbf B+\mathbf E,
\label{eq:mlm2}
\end{equation}
where \(\stackrel{n \times p}{\mathbf Y}=[\mathbf y_1, \ldots , \mathbf y_n]^\top\) is the data matrix for the \(y\)-variables, \(\mathbf X=[\mathbf x_1, \ldots , \mathbf x_n]^\top\) is the \(n \times q\) data matrix for the \(x\)-variables, defined as in \S 8.2, and \(\stackrel{n \times p}{\mathbf E}=[{\pmb \epsilon}_1, \ldots , {\pmb \epsilon}_n]^\top\).
...
Under this MVN assumption, the log-likelihood function \(\ell(\mathbf B, \boldsymbol{\Sigma})\) for the parameter matrices \(\mathbf B\) and \(\boldsymbol{\Sigma}\) is given by
\begin{align}
\ell(\mathbf B, \boldsymbol{\Sigma})&=-\frac{np}{2}\log(2\pi) -\frac{n}{2}\log(\vert \boldsymbol{\Sigma}\vert) \nonumber \\
...
\begin{proposition}
\protect\hypertarget{prp:eight1}{}{\label{prp:eight1} }The maximum likelihood estimators of \(\mathbf B\) and \(\boldsymbol{\Sigma}\) in \eqref{eq:MVNlik}
are given by
\begin{equation}
...
and
\begin{equation}
\hat{\boldsymbol{\Sigma}}=\frac{1}{n}\mathbf Y^\top \mathbf P\mathbf Y,
\label{eq:RSS}
\end{equation}
where \(\mathbf P\) is the matrix defined in \eqref{eq:defP}. The maximised log-likelihood is given by
\begin{equation}
\ell(\hat{\mathbf B}, \hat{\boldsymbol{\Sigma}})=- \frac{n}{2} \log(\vert \hat{\boldsymbol{\Sigma}} \vert )-\frac{np}{2}\log(2\pi) -\frac{np}{2}.
\label{eq:maxlik}
\end{equation}
\end{proposition}

\textbf{Remark:} note how similar \eqref{eq:MVbeta} and \eqref{eq:RSS} are to their univariate counterparts \eqref{eq:uni1} and \eqref{eq:uni2}, respectively; the only thing that is different is that \(\mathbf Y\) is now an \(n \times p\) matrix rather than an \(n \times 1\) vector.

\begin{proof}
\iffalse{} {Proof. } \fi{}\textbf{of \eqref{eq:MVbeta}\}.} Recall that \(\mathbf P\) defined in \eqref{eq:defP} is a projection matrix, i.e. \(\mathbf P^\top =\mathbf P\) and \(\mathbf P^2 = \mathbf P\).
Moreover,
\begin{equation}
\mathbf P\mathbf X=\mathbf X-\mathbf X(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf X=\mathbf X-\mathbf X={\mathbf 0}_{n,q}
\label{eq:orthog27a}
\end{equation}
and
\begin{equation}
\mathbf X^\top \mathbf P=\mathbf X^\top - \mathbf X^\top \mathbf X(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top =\mathbf X^\top -\mathbf X^\top ={\mathbf 0}_{q,n}.
\label{eq:orthog27b}
\end{equation}
Now write
\begin{align*}
\mathbf Y-\mathbf X\mathbf B&=\mathbf Y-\mathbf X\hat{\mathbf B} +\mathbf X\hat{\mathbf B}-\mathbf X\mathbf B\\
&=\mathbf Y-\mathbf X(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf Y+\mathbf X(\hat{\mathbf B}-\mathbf B)\\
&=\mathbf P\mathbf Y+\mathbf X(\hat{\mathbf B}-\mathbf B).
\end{align*}
Then, using \eqref{eq:orthog27a} and \eqref{eq:orthog27b},
\begin{align}
(\mathbf Y&- \mathbf X\mathbf B)^\top (\mathbf Y- \mathbf X\mathbf B) \nonumber \\
&=\{\mathbf P\mathbf Y+ \mathbf X(\hat{\mathbf B}-\mathbf B)\}^\top \{\mathbf P\mathbf Y+\mathbf X(\hat{\mathbf B}-\mathbf B)\}\nonumber \\
&=\mathbf Y^\top \mathbf P\mathbf Y+ \mathbf Y^\top \mathbf P\mathbf X(\hat{\mathbf B}-\mathbf B)\nonumber \\
& \qquad \qquad + (\hat{\mathbf B}-\mathbf B)^\top \mathbf X^\top \mathbf P\mathbf Y+(\hat{\mathbf B}-\mathbf B)^\top \mathbf X^\top \mathbf X(\hat{\mathbf B}-\mathbf B)
\nonumber\\
&=\mathbf Y^\top \mathbf P\mathbf Y+ (\hat{\mathbf B}-\mathbf B)^\top \mathbf X^\top \mathbf X(\hat{\mathbf B}-\mathbf B).
\label{eq:decomp87}
\end{align}
As noted in Chapter 2, for any compatible matrices \(\mathbf W\) and \(\mathbf Z\), we have \(\text{tr}(\mathbf W\mathbf Z)=\text{tr}(\mathbf Z\mathbf W)\). So, using \eqref{eq:decomp87}, it follows that the trace term in \eqref{eq:MVNlik} may be written
\begin{align}
&\text{tr}\left \{ (\mathbf Y-\mathbf X\mathbf B)\boldsymbol{\Sigma}^{-1} (\mathbf Y- \mathbf X\mathbf B)^\top \right \}\nonumber \\
&=\text{tr}\left \{\boldsymbol{\Sigma}^{-1} (\mathbf Y-\mathbf X\mathbf B)^\top (\mathbf Y- \mathbf X\mathbf B) \right \}\nonumber \\
&=\text{tr}\left \{  \boldsymbol{\Sigma}^{-1} ( \mathbf Y-\mathbf X\hat{\mathbf B}+\mathbf X\hat{\mathbf B} -\mathbf X\mathbf B)^\top
( \mathbf Y-\mathbf X\hat{\mathbf B}+\mathbf X\hat{\mathbf B} -\mathbf X\mathbf B)       \right \}\nonumber \\
&= \text{tr}\left [\boldsymbol{\Sigma}^{-1} \{\mathbf Y^\top \mathbf P\mathbf Y+(\hat{\mathbf B}-\mathbf B)^\top \mathbf X^\top \mathbf X(\hat{\mathbf B}-\mathbf B)  \}\right ]
\nonumber\\
&=\text{tr} \left (\boldsymbol{\Sigma}^{-1}\mathbf Y^\top \mathbf P\mathbf Y\right ) + \text{tr}\left \{
\mathbf X(\hat{\mathbf B}-\mathbf B) \boldsymbol{\Sigma}^{-1} (\hat{\mathbf B}-\mathbf B)^\top  \mathbf X^\top  \right \}
\label{eq:twobits}
\end{align}
The first term on the RHS of \eqref{eq:twobits} does not depend on \(\mathbf B\). In the second term, the matrix inside the trace is non-negative definite for all \(\mathbf B\), and therefore the second term in \eqref{eq:twobits} is non-negative, and has a minimum value of \(0\), achieved uniquely when \(\mathbf B=\hat{\mathbf B}\). Therefore, for fixed \(\boldsymbol{\Sigma}\), \eqref{eq:MVNlik} is maximised when \(\mathbf B=\hat{\mathbf B}\), as defined in \eqref{eq:MVbeta}. \hfill \(\square\)

To prove \eqref{eq:RSS}, we require the following result, whose proof is omitted because it is a bit too technical.
\end{proof}
\begin{proof}
\iffalse{} {Proof. } \fi{}Suppose that \(\mathbf A\) is a fixed symmetric positive definite \(p \times p\) matrix and define the function
\[
G(\boldsymbol{\Sigma}) \equiv -\frac{n}{2} \log \left (\vert \boldsymbol{\Sigma}\vert\right)  -\frac{1}{2}\text{tr} \left (\boldsymbol{\Sigma}^{-1} \mathbf A\right).
\]
Then \(G(\boldsymbol{\Sigma})\) is maximised over symmetric positive definite \(p \times p\) matrices \(\boldsymbol{\Sigma}\) at
\(\hat{\boldsymbol{\Sigma}}=n^{-1}\mathbf A\), and the maximum value of \(G\) is given by
\[
G(\hat{\boldsymbol{\Sigma}})= -\frac{n}{2} \log \left (\vert n^{-1} \mathbf A\vert \right ) -np/2.
\]
\end{proof}

\begin{proof}
\iffalse{} {Proof. } \fi{}\textbf{of \eqref{eq:RSS} and \eqref{eq:maxlik}\}.} Again using the result \(\text{tr}(\mathbf W\mathbf Z)=\text{tr}(\mathbf Z\mathbf W)\) for compatible matrices \(\mathbf W\) and \(\mathbf Z\), we have
\[
\text{tr}\{(\mathbf Y-\mathbf X\hat{\mathbf B}) \boldsymbol{\Sigma}^{-1}(\mathbf Y-\mathbf X\hat{\mathbf B})^\top\}=\text{tr}(\boldsymbol{\Sigma}^{-1}(\mathbf P\mathbf Y)^\top \mathbf P\mathbf Y)=\text{tr}(\boldsymbol{\Sigma}^{-1} \mathbf Y^\top \mathbf P\mathbf Y),
\]
because \(\mathbf P\) is a projection matrix. Consequently,
\[
\ell(\hat{\mathbf B},\boldsymbol{\Sigma})=-\frac{np}{2}\log(2\pi)-\frac{n}{2}\log \vert \boldsymbol{\Sigma}\vert -\frac{1}{2} \text{tr}
\left \{ \boldsymbol{\Sigma}^{-1}\mathbf Y^\top \mathbf P\mathbf Y\right \},
\]
and we want to maximise \(\ell(\hat{\mathbf B}, \boldsymbol{\Sigma})\) over symmetric positive definite matrices \(\boldsymbol{\Sigma}\). So take \(\mathbf A=\mathbf Y^\top \mathbf P\mathbf Y\). Then, using Proposition \ref{prp:eight2}, \(\hat{\boldsymbol{\Sigma}} = n^{-1}\mathbf Y^\top \mathbf P\mathbf Y\), which agrees with \eqref{eq:RSS}.

To prove \eqref{eq:maxlik}, note that
\begin{align*}
\text{tr}\{(\mathbf Y-\mathbf X\hat{\mathbf B}) \hat{\boldsymbol{\Sigma}}^{-1}(\mathbf Y-\mathbf X\hat{\mathbf B})^\top \}
&=\text{tr}\{\hat{\boldsymbol{\Sigma}}^{-1} (\mathbf P\mathbf Y)^\top \mathbf P\mathbf Y\}\\
&= n \text{tr}\{(\mathbf Y^\top \mathbf P\mathbf Y)^{-1} \mathbf Y^\top \mathbf P\mathbf Y\}\\
&=n\text{tr}(\mathbf I_p)=np,
\end{align*}
so \eqref{eq:maxlik} follows after substitution of \(\mathbf B=\hat{\mathbf B}\) and \(\boldsymbol{\Sigma}=\hat{\boldsymbol{\Sigma}}\) into \eqref{eq:MVNlik}.
\end{proof}

We now determine the joint distribution of \(\hat{\mathbf B}\) and \(\hat{\boldsymbol{\Sigma}}\).

\begin{proposition}
\protect\hypertarget{prp:eight3}{}{\label{prp:eight3} }Assume that \eqref{eq:mlm2} and \eqref{eq:MVNassumption} both hold and assume that \(\mathbf X\) has full rank \(q\), where \(q < n\). Then the following results hold.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  The MLE of \(\mathbf B\), \(\hat{\mathbf B}\) defined in \eqref{eq:MVbeta}, and the MLE of \(\boldsymbol{\Sigma}\), \(\hat{\boldsymbol{\Sigma}}\) defined in \eqref{eq:RSS},
  are independent. Moreover, the elements of \(\hat{\mathbf B}\) are jointly multivariate normal, while \(n\hat{\boldsymbol{\Sigma}} \sim W_p(\boldsymbol{\Sigma}, n-q)\).
\item
  The MLE \(\hat{\mathbf B}\) satisfies \(E[\hat{\mathbf B}]=\mathbf B\), i.e. \(\hat{\mathbf B}\) is unbiased for \(\mathbf B\).
\end{enumerate}

3.Write \(\hat{\mathbf b}_{[j]}\) for column \(j\) of \(\hat{\mathbf B}\), \(j=1, \ldots , p\), so that \(\hat{\mathbf B}=[\hat{\mathbf b}_{[1]}, \ldots , \hat{\mathbf b}_{[p]}]\).
Then for \(j,k=1, \ldots , p\),
\[
\text{Cov}(\hat{\mathbf b}_{[j]}, \hat{\mathbf b}_{[k]})=\sigma_{jk} (\mathbf X^\top \mathbf X)^{-1}.
\]
\end{proposition}

\begin{proof}
\iffalse{} {Proof. } \fi{}\emph{of Proposition \ref{prp:eight3}}
First, observe that \(\hat{\boldsymbol{\Sigma}}\) is a function of the elements of \(\mathbf P\mathbf Y\), namely
\[
\hat{\boldsymbol{\Sigma}}=n^{-1} \mathbf Y^\top \mathbf P^\top \mathbf P\mathbf Y=n^{-1}\mathbf Y^\top \mathbf P\mathbf Y,
\]
because \(\mathbf P\) is a projection matrix. Moreover, under the MVN assumption \eqref{eq:MVNassumption}, \(\mathbf P\mathbf Y\) and
\(\hat{\mathbf B}\) are jointly MVN, so to prove part 1. it will be sufficient to prove that each column of \(\hat{\mathbf B}\) is uncorrelated with
each column of \(\mathbf P\mathbf Y\). Write \(\mathbf y_{[j]}\) for column \(j\) of \(\mathbf Y\). Then
\[
\mathbf Y=[\mathbf y_{[1]}, \ldots , \mathbf y_{[p]}], \qquad \mathbf P\mathbf Y=[\mathbf P\mathbf y_{[1]}, \ldots , \mathbf P\mathbf y_{[p]}],
\]
and
\[
\hat{\mathbf B}\equiv [\hat{\mathbf b}_{[1]}, \ldots , \hat{\mathbf b}_{[p]}] =[(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf y_{[1]}, \ldots , (\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf y_{[p]}],
\]
where, as before, \(\hat{\mathbf b}_{[j]}\) is column \(j\) of \(\hat{\mathbf B}\). The covariance between column \(j\) of \(\hat{\mathbf B}\) and column \(k\) of \(\mathbf P\mathbf Y\) is given by
\begin{align*}
\text{Cov}(\hat{\mathbf b}_{[j]}, \mathbf P\mathbf y_{[k]})&= \text{Cov}\{(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf y_{[j]}, \mathbf P\mathbf y_{[k]}\}\\
&=(\mathbf X^\top \mathbf X)^{-1} \mathbf X^\top \text{Cov}(\mathbf y_{[j]}, \mathbf y_{[k]})\mathbf P\\
&=(\mathbf X^\top \mathbf X)^{-1} \mathbf X^\top \sigma_{jk}\mathbf I_n\mathbf P\\
&=\sigma_{jk}(\mathbf X^\top \mathbf X)^{-1} \mathbf X^\top (\mathbf I_n - \mathbf X(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top)={\mathbf 0}_{q,n},
\end{align*}
where \(\boldsymbol{\Sigma}=(\sigma_{jk})_{j,k=1}^p\). In the above we have used the definition of \(\mathbf P\) in \eqref{eq:defP} and the fact that
\begin{equation}
\text{Cov}(\mathbf y_{[j]}, \mathbf y_{[k]})=\sigma_{jk}\mathbf I_n
\label{eq:energise}
\end{equation}
due to the independence of the rows of \(\mathbf Y\). Therefore \(\hat{\mathbf B}\) and \(\mathbf P\mathbf Y\) are independent by Proposition \ref{prp:six4}, and
...
\begin{align*}
E[\hat{\mathbf B}]&=E[(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf Y]\\
&=(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top E[\mathbf Y]\\
&=(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf X\mathbf B=\mathbf B.
\end{align*}

For part 3., using \eqref{eq:energise} again,
\begin{align*}
\text{Cov}(\hat{\mathbf b}_{[j]}, \hat{\mathbf b}_{[k]})&=\text{Cov}\{(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \mathbf y_{[j]}, (\mathbf X^\top \mathbf X)^{-1} \mathbf X^\top \mathbf y_{[k]}\}\\
&=(\mathbf X^\top \mathbf X)^{-1}\mathbf X^\top \text{Cov}(\mathbf y_{[j]}, \mathbf y_{[k]}) \mathbf X(\mathbf X^\top \mathbf X)^{-1}\\
&=(\mathbf X^\top \mathbf X)^{-1} \mathbf X^\top \sigma_{jk}\mathbf I_n\mathbf X(\mathbf X^\top \mathbf X)^{-1}=\sigma_{jk}(\mathbf X^\top \mathbf X)^{-1},
\end{align*}
as required.\\
\end{proof}
...
\[
H_0: \, \mathbf B=\begin{pmatrix}\mathbf B^\ast \\{\mathbf 0}_{q-r,p} \end{pmatrix}, \qquad \boldsymbol{\Sigma}\quad \text{unrestricted},
\]
where \(\mathbf B^\ast\) is \(r \times p\) with \(1 \leq r <q\). The alternative hypothesis is
\[
H_1: \, \stackrel{q \times p}{\mathbf B} \quad \text{unrestricted}, \qquad \boldsymbol{\Sigma}\quad
\text{unrestricted},
\]
i.e.~model \eqref{eq:mlm2} under the MVN assumption \eqref{eq:MVNassumption}. Note that \(H_0\) is nested within \(H_1\).
Let us write \(\hat{\mathbf B}_j\) and \(\hat{\boldsymbol{\Sigma}}_j\) for the MLEs of \(\mathbf B\) and \(\boldsymbol{\Sigma}\) under hypothesis \(H_j\), \(j=0,1\). Then, using \eqref{eq:maxlik} in Proposition \ref{prp:eight1} to calculate the maximised likelihood under \(H_0\) and \(H_1\), we obtain the Wilks statistic, \(\omega_{0,1}\) (equals ``twice the different in maximised log likelihoods''):
\begin{align*}
\omega_{0,1}&=2\{\ell(\hat{\mathbf B}_1, \hat{\boldsymbol{\Sigma}}_1)-\ell(\hat{\mathbf B}_0, \hat{\Sigma}_0)\}\\
...
\begin{equation}
 ...
 \label{eq:MANOVA1}
 \end{equation}
where the \({\pmb \epsilon}_{ij}\) are IID \(N_p({\mathbf 0}_p, \boldsymbol{\Sigma})\), and the \(\mathbf y_{ij}\) and \({\boldsymbol{\mu}}_j\) are also
\(p \times 1\) vectors. It is assumed that \(\text{Var}({\pmb \epsilon}_i)=\boldsymbol{\Sigma}\) is constant across the \(t\) populations.

One possibility is to use the linear model framework developed in \S 8.3. In this case, \(\mathbf X\) is a matrix with each element equal to zero or one. However, it is also feasible to do the calculations directly. This is what we shall do.
...
Using results in \S 2.10, the partial derivative, or gradient, of \eqref{eq:MANOVAlik} with respect to the vector \({\boldsymbol{\mu}}_k\) is given by
\begin{align*}
 ...
 \end{align*}
where \(\bar{\mathbf y}_{+k}\) is the sample mean of group \(k\), i.e.
\[
\bar{\mathbf y}_{+k}=n_k^{-1}\sum_{i=1}^{n_k} \mathbf y_{ik}.
\]
So, setting \(\partial \ell/\partial {\boldsymbol{\mu}}_k={\mathbf 0}_p\) implies \(\hat{{\boldsymbol{\mu}}}_k=\bar{\mathbf y}_{+k}\).
Therefore
\begin{align*}
...
\]
Under \(H_0\), the \(\mathbf y_{ij}\) are IID \(N_p({\boldsymbol{\mu}}, \boldsymbol{\Sigma})\), so the log-likelihood under \(H_0\) is
\begin{align*}
...
\]
where \(\mathbf T\) is the matrix analogue of the total sum of squares, i.e.
\[
...
\]
where \(\mathbf B\) is the matrix analogue of the `between' sum of squares \(B\) in @ref(eq:T=B+W), i.e.
\[
...
\item
  \href{https://mediaspace.nottingham.ac.uk/media/LDAA+Bayes+discriminant+rule/1_x48emgqz}{Bayes discriminant rule}
\item
  \href{https://mediaspace.nottingham.ac.uk/media/LDAA+Iris+example/1_cb9grj25}{Example: Iris data}
\item
  \href{https://mediaspace.nottingham.ac.uk/media/LDAA+Fisher\%27s+LDA/1_yl7qpbw9}{Fisher's LDA}\\
\item
...
\begin{align*}
(\mathbf x-{\boldsymbol{\mu}}_k)^\top \boldsymbol{\Sigma}^{-1} (\mathbf x-{\boldsymbol{\mu}}_k) &= \mathbf x^\top \boldsymbol{\Sigma}\mathbf x
-2{\boldsymbol{\mu}}_k^\top \boldsymbol{\Sigma}^{-1}\mathbf x+{\boldsymbol{\mu}}_k^\top \boldsymbol{\Sigma}^{-1}{\boldsymbol{\mu}}_k\\
&= \mathbf x^\top \boldsymbol{\Sigma}\mathbf x-\delta_k(\mathbf x)
\end{align*}
Thus,
