...
Suppose we have a sample of \(n\) vectors \(\mathbf x_1, \ldots , \mathbf x_n \in \mathbb{R}^p\). Consider the situation where \(\mathbf x_i\) comes from one of \(K\) sub-populations (I used \(g\) previously, but this method is known as `K'-means so we'll use \(K\) instead of \(g\) here). We'll initially assume \(K\) is known, but will discuss how to choose \(K\) in applications where we do not have prior beliefs about the number of clusters.
...
We think of the encoder \(\delta(i)\) as a parameter that we need to estimate. We estimate it by picking a loss function, and then seeking to minimize that loss. A natural choice for the loss function is to use the within-cluster scatter that we saw previously:

\[W(\delta) = \sum_{k=1}^K \sum_{i: \delta(i)=k} \sum_{i': \delta(i')=k} d(\mathbf x_i, \mathbf x_{i'})\]
...
\sum_{k=1}^K \sum_{i=1}^n \sum_{i'=1}^n d(\mathbf x_i, \mathbf x_{i'})&= \sum_{k=1}^K \sum_{i: \delta(i)=k} \left(\sum_{i': \delta(i')=k} d(\mathbf x_i, \mathbf x_{i'})+\sum_{i': \delta(i')\not=k} d(\mathbf x_i, \mathbf x_{i'})\right)
\end{align}
...
\begin{align}
W(\delta) &= \sum_{k=1}^K \sum_{i: \delta(i)=k} \sum_{i': \delta(i')=k} ||\mathbf x_i - \mathbf x_{i'}||_2^2 \\
&= \sum_{k=1}^K 2n_k \sum_{i: \delta(i)=k} ||\mathbf x_{i} - \hat{{\boldsymbol{\mu}}}_k||_2^2 \label{eq:Wscatterk}\\
\end{align}
...
Note that the result of these steps might be a sub-optimal local minimum. Thus, we usually run \(K\)-means several times with different random initial choices for \(\delta\), and then choose the best solution.
...
\includegraphics{10-clustering_files/figure-latex/unnamed-chunk-3-1.pdf}
...
\includegraphics{10-clustering_files/figure-latex/unnamed-chunk-4-1.pdf}
...
\includegraphics{10-clustering_files/figure-latex/unnamed-chunk-6-1.pdf}
...
&=\left \{\prod_{i: \delta_i=1} f(\mathbf x_i; \boldsymbol \theta_1)\right \}\cdots \left \{\prod_{i: \delta_i=} f(\mathbf x_i; \boldsymbol \theta_K)\right \}\\
...
\begin{proposition}
\protect\hypertarget{prp:ten1}{}{\label{prp:ten1} }If \(\mathbf x\in \hat{\mathcal{C}}_j\) then
\[
\sup_{1 \leq k \leq K}f(\mathbf x; \hat{\boldsymbol \theta}_k) = f(\mathbf x; \hat{\boldsymbol \theta}_j).
\]
\end{proposition}

\begin{proof}
\iffalse{} {Proof. } \fi{}If we move an \(\mathbf x\) from \(\hat{\mathcal{C}}_j\) to \(\hat{\mathcal{C}}_k\), then the likelihood changes to
\[
L(\hat{\boldsymbol \theta}_1, \ldots , \hat{\boldsymbol \theta}_K; \boldsymbol \delta)f(\mathbf x; \hat{\boldsymbol \theta}_k)/f(\mathbf x; \hat{\boldsymbol \theta}_j).
\]
But by definition of \(L(\hat{\boldsymbol \theta}_1, \ldots , \hat{\boldsymbol \theta}_K; \hat{\boldsymbol \delta})\),
\[
L(\hat{\boldsymbol \theta}_1, \ldots , \hat{\boldsymbol \theta}_K; \hat{\boldsymbol \delta})f(\mathbf x; \hat{\boldsymbol \theta}_k)/f(\mathbf x; \hat{\boldsymbol \theta}_j) \leq L(\hat{\boldsymbol \theta}_1, \ldots , \hat{\boldsymbol \theta}_K; \hat{\boldsymbol \delta}),
\]
...
\[
\prod_{i=1}^n\exp(-\frac{1}{2\sigma}||\mathbf x_i-{\boldsymbol{\mu}}_{\delta(i)}||_2^2)
\]
...
\includegraphics{10-clustering_files/figure-latex/unnamed-chunk-9-2.pdf}
...
\includegraphics{10-clustering_files/figure-latex/unnamed-chunk-16-1.pdf}
...
\includegraphics{10-clustering_files/figure-latex/unnamed-chunk-21-1.pdf}
...
\includegraphics{10-clustering_files/figure-latex/unnamed-chunk-22-1.pdf}
